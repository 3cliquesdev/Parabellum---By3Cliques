...
DROP FUNCTION IF EXISTS public.get_commercial_conversations_report(timestamp with time zone, timestamp with time zone, uuid, uuid, text, text, text, integer, integer);
...
RETURNS TABLE(
  ...
  human_first_response_seconds bigint,
  human_resolution_seconds bigint,
  total_count bigint
)
LANGUAGE sql
STABLE
...
AS $function$
  SELECT
    ...
    handoff_calc.handoff_at,
    handoff_calc.human_first_response_seconds,
    handoff_calc.human_resolution_seconds,
  ...
  LEFT JOIN LATERAL (
    SELECT
      ho.handoff_at,
      CASE
        WHEN ho.handoff_at IS NOT NULL AND hfr.first_human_msg_at IS NOT NULL
          THEN EXTRACT(EPOCH FROM (hfr.first_human_msg_at - ho.handoff_at))::bigint
        ELSE NULL
      END AS human_first_response_seconds,
      CASE
        WHEN ho.handoff_at IS NOT NULL AND c.closed_at IS NOT NULL
          THEN EXTRACT(EPOCH FROM (c.closed_at - ho.handoff_at))::bigint
        ELSE NULL
      END AS human_resolution_seconds
    FROM (
      SELECT MIN(ae.created_at) AS handoff_at
      FROM ai_events ae
      WHERE ae.entity_id = c.id
        AND ae.entity_type = 'conversation'
        AND ae.event_type IN ('state_transition_handoff_to_human', 'ai_transfer', 'handoff_to_human')
    ) ho
    LEFT JOIN LATERAL (
      SELECT MIN(m.created_at) AS first_human_msg_at
      FROM messages m
      WHERE m.conversation_id = c.id
        AND m.sender_type::text IN ('agent', 'user')
        AND m.sender_id IS NOT NULL
        AND m.created_at > ho.handoff_at
    ) hfr ON true
  ) handoff_calc ON true
