...
BEGIN
  ...
  -- For entries WITH conversation_id: count distinct conversations (preferred)
  -- For legacy entries WITHOUT conversation_id: sample latest per user per hour to reduce noise
  sentiment_with_conv AS (
    SELECT 
      CASE 
  ...
  sentiment_legacy AS (
    ...
    WHERE al.feature_type = 'sentiment'
      ...
      AND al.result_data->>'sentiment' IS NOT NULL
      AND (al.result_data->>'fallback' IS NULL OR al.result_data->>'fallback' != 'true')
      AND al.conversation_id IS NULL
    ORDER BY al.user_id, date_trunc('hour', al.created_at), al.created_at DESC
  ),
  sentiment_combined AS (
    SELECT normalized_sentiment FROM sentiment_with_conv
    UNION ALL
    SELECT normalized_sentiment FROM sentiment_legacy
  ),
  sentiment_stats AS (
    SELECT
      jsonb_object_agg(
