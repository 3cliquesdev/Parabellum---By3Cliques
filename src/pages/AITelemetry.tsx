...
import { useAIDecisionTelemetry, REASON_LABELS, TRANSITION_LABELS, type TransferEvent } from "@/hooks/useAIDecisionTelemetry";
...
import {
  Brain, AlertTriangle, ArrowRightLeft, ShieldAlert, Activity, RefreshCw, Check,
  ArrowUpDown, Copy, Route, Tag, XCircle, CheckCircle2
} from "lucide-react";
...
const TRANSFER_COLORS: Record<string, string> = {
  "Handoff → Humano": "#f59e0b",
  "Atribuir Agente": "#10b981",
  "Desatribuir Agente": "#ef4444",
  "Engajar IA": "#6366f1",
  "Setar Copilot": "#8b5cf6",
  "Atualizar Dept": "#0ea5e9",
  "Encerrar": "#6b7280",
};
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="decisions" className="w-full">
        ...
        {/* ===== TAB: Transferências & Roteamento ===== */}
        <TabsContent value="transfers" className="space-y-6 mt-0">
          {/* Transfer KPIs */}
          {isLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {Array.from({ length: 4 }).map((_, i) => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <Card className="border-border">
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-muted-foreground">Transições (24h)</p>
                      <p className="text-3xl font-bold text-foreground">{transferKpis.totalTransfers}</p>
                    </div>
                    <div className="h-10 w-10 rounded-full bg-muted flex items-center justify-center">
              ...
              <Card className="border-amber-400/20 bg-amber-400/5">
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-muted-foreground">Handoffs → Humano</p>
                      <p className="text-3xl font-bold text-amber-400">{transferKpis.handoffsToHuman}</p>
                      {transferKpis.deptChanges > 0 && (
                        <p className="text-[10px] text-muted-foreground mt-0.5">
                          {transferKpis.deptChanges} com troca de dept
                        </p>
                      )}
                    </div>
                    <div className="h-10 w-10 rounded-full bg-amber-400/10 flex items-center justify-center">
              ...
              <Card className={transferKpis.sameDeptTransfers > 0 ? "border-red-400/20 bg-red-400/5" : "border-border"}>
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-muted-foreground">Sem troca de dept</p>
                      <p className={`text-3xl font-bold ${transferKpis.sameDeptTransfers > 0 ? "text-red-400" : "text-foreground"}`}>
                        {transferKpis.sameDeptTransfers}
                      </p>
                      <p className="text-[10px] text-muted-foreground mt-0.5">
                        Possíveis mismatches
                      </p>
                    </div>
                    <div className={`h-10 w-10 rounded-full flex items-center justify-center ${transferKpis.sameDeptTransfers > 0 ? "bg-red-400/10" : "bg-muted"}`}>
                      <AlertTriangle className={`h-5 w-5 ${transferKpis.sameDeptTransfers > 0 ? "text-red-400" : "text-muted-foreground"}`} />
                    </div>
                  </div>
                </CardContent>
              </Card>
              <Card className={transferKpis.closesWithoutTag > 0 ? "border-orange-400/20 bg-orange-400/5" : "border-border"}>
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-muted-foreground">Closes sem tag</p>
                      <p className={`text-3xl font-bold ${transferKpis.closesWithoutTag > 0 ? "text-orange-400" : "text-foreground"}`}>
                        {transferKpis.closesWithoutTag}
                      </p>
                      {transferKpis.proactiveCloses > 0 && (
                        <p className="text-[10px] text-green-500 mt-0.5">
                          {transferKpis.proactiveCloses} proativos
                        </p>
                      )}
                    </div>
                    <div className={`h-10 w-10 rounded-full flex items-center justify-center ${transferKpis.closesWithoutTag > 0 ? "bg-orange-400/10" : "bg-muted"}`}>
                      <Tag className={`h-5 w-5 ${transferKpis.closesWithoutTag > 0 ? "text-orange-400" : "text-muted-foreground"}`} />
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium">Distribuição de transições</CardTitle>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                <Skeleton className="h-[220px] w-full" />
                ...
                <div className="h-[220px] flex items-center justify-center text-sm text-muted-foreground">
                  Nenhuma transição nas últimas 24h
                </div>
              ) : (
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={transferBreakdown} layout="vertical">
                    <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                    <XAxis type="number" tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} allowDecimals={false} />
                    <YAxis
                      dataKey="name"
                    ...
                    <Tooltip
                      ...
                      formatter={(value: number) => [value, "Transições"]}
                    />
                    <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                      {transferBreakdown.map((entry, i) => (
                        <Cell key={i} fill={TRANSFER_COLORS[entry.name] || "#6b7280"} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium flex items-center gap-2">
                ...
                Transições Recentes
                <Badge variant="secondary" className="ml-1">Últimas 50</Badge>
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 px-4">
                  ...
                  <p className="text-base font-medium text-foreground">Nenhuma transição registrada</p>
                  <p className="text-sm text-muted-foreground mt-1">As transições aparecerão aqui conforme conversas forem roteadas.</p>
                </div>
              ) : (
                <div className="overflow-auto max-h-[420px]">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead>Conversa</TableHead>
                        <TableHead>Tipo</TableHead>
                        <TableHead>Dept Origem</TableHead>
                        <TableHead>Dept Destino</TableHead>
                          ...
                          <TableRow key={evt.id} className="hover:bg-muted/50">
                            ...
                            <TableCell>
                              <span
                                className="inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium"
                                style={{
                                  borderColor: (TRANSFER_COLORS[label] || "#6b7280") + "30",
                                  backgroundColor: (TRANSFER_COLORS[label] || "#6b7280") + "15",
                                  color: TRANSFER_COLORS[label] || "#6b7280",
                                }}
                              >
                                {label}
                              </span>
                            ...
                            <TableCell className="text-xs text-muted-foreground truncate max-w-[150px]" title={json?.reason || ""}>
                              {json?.reason || "—"}
                            </TableCell>
                            <TableCell className="text-xs text-muted-foreground whitespace-nowrap">
                              {formatDistanceToNow(new Date(evt.created_at), { addSuffix: true, locale: ptBR })}
